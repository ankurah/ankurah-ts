            ...
            syn::Item::Mod(m) => {
                ...
                // serde's `ser` and `de` — are nothing but impls and traits.
                    ...
                    if let Some((_, items)) = &m.content {
                        ...
                        // G5: the sub-file carries the PATH its module lives
                        // at; see `extract::inline`.
                        let mut sub_file =
                            RustFile::empty(inline_module_path(&file.path, &mod_name, m.ident.span()));
...
fn extract_impl(i: &syn::ItemImpl, cfg: ExtractCfg) -> ImplInfo {
    ...
    let const_items = i
        ...
        .filter_map(|item| match item {
            syn::ImplItem::Const(c) if !is_skipped_cfg_with(&c.attrs, cfg.features) => {
                Some((c.ident.to_string(), syn::spanned::Spanned::span(c)))
            }
            _ => None,
        })
        .collect();
