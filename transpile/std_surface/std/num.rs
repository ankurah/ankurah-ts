//! The inherent methods of the numeric primitives, plus `std::num`'s errors.
//!
//! `impl u64 { .. }` is not writable outside core; syn parses it and it is
//! where these methods honestly belong. Only the widths and methods the corpus
//! reaches are written out, plus the obvious neighbours of each — the byte
//! conversions the collation code uses, the saturating and wrapping arithmetic,
//! and the float predicates. Everything else is a declared gap.
...
impl Display for ParseIntError { fn fmt(&self, f: &mut Formatter<'_>) -> std::fmt::Result { todo!() } }
...
impl Display for ParseFloatError { fn fmt(&self, f: &mut Formatter<'_>) -> std::fmt::Result { todo!() } }
...
impl Display for TryFromIntError { fn fmt(&self, f: &mut Formatter<'_>) -> std::fmt::Result { todo!() } }
