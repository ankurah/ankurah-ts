    ...
    try {
      ...
      try {
        ...
        try {
          ...
          const _seq8 = materialized;
          let _at9 = 0;
          try {
            while (_at9 < _seq8.length) {
              const [name, value, isJsonb] = _seq8[_at9++];
              columns.push(name);
              values.push((() => {
                if (value != null) {
                  const v = value;
          ...
          const _r14 = await conn.withConnection(new OwnedClosure([newHead], (c) => {
            const _m10 = (() => {
  ...
  async getEvents(eventIds: EventId[]): Promise<Result<Attested<Event>[], RetrievalError>> {
    try {
      ...
      try {
        ...
        })).mapErr((e) => new RetrievalError('StorageError', { _0: e }));
      } finally {
        dropOwned(conn);
      }
    } finally {
