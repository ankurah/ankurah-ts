...
class ReactorSubInner {
   ...
   * Divergence: JS has no Drop; callers must invoke dispose() explicitly or via Symbol.dispose [E11].
   */
  dispose(): void {
    if (!this.disposed) {
      this.disposed = true;
...
/**
 * A handle to a reactor subscription that automatically cleans up on dispose.
 *
